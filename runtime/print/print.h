...
namespace runtime
{
    inline void f_print(const std::vector<Object> &args, const bool newline)
    {
        // Don't waste memory if a newline is not needed
        if (!newline)
        {
            fluent_std::Runtime::print(static_cast<char *>(args[0].value));
            return;
        }
    ...
    inline void print(const std::vector<Object> &args)
    {
        f_print(args, false);
    }

    inline void println(const std::vector<Object> &args)
    {
        f_print(args, true);
    }
}

#endif //RUNTIME_PRINT_H
