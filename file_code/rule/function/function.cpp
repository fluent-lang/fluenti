...
void process_function(file_code::FileCode &result, const std::shared_ptr<fluent::parser::AST> &ast)
{
    ...
    // Add all parameters
    for (
        const auto &params_children = fluent::util::try_unwrap(params_node->children);
        const auto &param : params_children
    )
    {
        // Get the param's children
        const auto param_children = fluent::util::try_unwrap(param->children);

        // Get the param's name
        const auto param_name = fluent::util::try_unwrap(param_children[0]->value);
    ...
    for (size_t i = 4; i < children.size(); i++)
    {
        ...
        const auto block_children = fluent::util::try_unwrap(block->children);

        // Get the block's name
        const auto block_name = fluent::util::try_unwrap(block_children[0]->value);
